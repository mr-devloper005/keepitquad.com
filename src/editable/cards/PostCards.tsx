...
export function EditorialFeatureCard({ post, href, label = 'Featured pick' }: { post: SitePost; href: string; label?: string }) {
  return (
    <Link href={href} className={`group block min-w-0 overflow-hidden ${dc.surface.dark} ${dc.motion.lift}`}>
      <div className="relative min-h-[480px] p-7 sm:p-9 lg:min-h-[560px]">
        ...
        <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(10,15,12,0.1),rgba(10,15,12,0.9))]" />
        <div className="relative z-10 flex h-full min-h-[420px] flex-col justify-end lg:min-h-[500px]">
          <span className="inline-flex w-fit items-center gap-2 rounded-full bg-[var(--slot4-accent-fill)] px-3.5 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--slot4-on-accent)]">{label}</span>
          <h3 className="editable-display mt-5 max-w-3xl text-3xl font-bold leading-[1.05] tracking-[-0.02em] sm:text-4xl lg:text-5xl">{post.title}</h3>
          <p className="mt-4 max-w-2xl text-sm leading-7 text-white/75 sm:text-base">{getEditableExcerpt(post, 190)}</p>
          <span className="mt-7 inline-flex w-fit items-center gap-2 text-sm font-semibold text-[var(--slot4-accent-bright)]">
  ...
  return (
    <Link href={href} className={`group ${dc.layout.minRailCard} block overflow-hidden ${dc.surface.card} ${dc.motion.lift}`}>
      <div className={`${dc.media.frame} ${dc.media.ratio}`}>
        ...
        <span className={`absolute left-3 top-3 rounded-full ${pal.accentBg} px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-[var(--slot4-on-accent)]`}>No. {String(index + 1).padStart(2, '0')}</span>
      </div>
      <div className="p-5">
        <p className={`text-[11px] font-semibold uppercase tracking-[0.2em] ${pal.accentText}`}>{getEditableCategory(post)}</p>
        <h3 className={`editable-display mt-2.5 line-clamp-3 text-lg font-bold leading-snug tracking-[-0.01em] ${pal.panelText}`}>{post.title}</h3>
        <p className={`mt-2.5 line-clamp-3 text-sm leading-6 ${pal.mutedText}`}>{getEditableExcerpt(post, 135)}</p>
      </div>
    </Link>
  )
}
  ...
  return (
    <Link href={href} className={`group grid min-w-0 gap-5 overflow-hidden ${dc.surface.card} p-4 ${dc.motion.lift} sm:grid-cols-[240px_minmax(0,1fr)]`}>
      ...
      <div className="min-w-0 p-2 sm:py-4 sm:pr-5">
        <p className={`text-[11px] font-semibold uppercase tracking-[0.2em] ${pal.accentText}`}>{getEditableCategory(post)} · {String(index + 1).padStart(2, '0')}</p>
        <h2 className={`editable-display mt-2.5 line-clamp-3 text-2xl font-bold leading-snug tracking-[-0.02em] ${pal.panelText} sm:text-3xl`}>{post.title}</h2>
        <p className={`mt-3 line-clamp-3 text-sm leading-7 ${pal.mutedText}`}>{getEditableExcerpt(post, 180)}</p>
        <span className={`mt-5 inline-flex items-center gap-2 text-sm font-semibold ${pal.accentText}`}>Open <ArrowRight className="h-4 w-4" /></span>
      </div>
    </Link>
  )
}
